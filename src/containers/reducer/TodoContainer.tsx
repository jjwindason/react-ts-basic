...
const TodoContainer = () => {
    ...
    // 添加任务
    const onAddTodo = (text: string) => dispatch({ type: 'add', payload: text })
    // 改变状态
    const onToggleTodo = (id: number) => dispatch({ type: 'toggle', payload: id })
    // 删除
    const onDeleteTodo = (id: number) => dispatch({ type: 'delete', payload: id })
    ...
    return (
        <section className="todoapp">
            ...
            {/* 提供共享数据 */}
            <TodoContext.Provider value={{ todos: state, onToggleTodo, onDeleteTodo }}>
                {/* 中间件，测试Context所用 */}
